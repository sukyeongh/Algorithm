const fs = require('fs');
const stdin = process.platform === 'linux' ? fs.readFileSync('/dev/stdin').toString().trim() : `16`;
...
const Moo = (curr_num, index, len) => {
  ...
  if (curr_num <= before) return Moo(curr_num, index - 1, before);
  else if (before < curr_num && curr_num <= before + (3 + index)) {
    if (curr_num === before + 1) return 'm';
    else return 'o';
  } else return Moo(curr_num - (before + (3 + index)), index - 1, before);
};
